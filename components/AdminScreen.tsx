...
export default function AdminScreen(props: AdminScreenProps) {
  ...
  // 棒グラフの角丸設定（分割代入の徹底）
  const [rTopLeft, rTopRight, rBottomRight, rBottomLeft] = BAR_RADIUS;
        ...
        <div className="space-y-6">
          ...
          {adminTab === 'stats' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="bg-white p-6 rounded-[2rem] shadow-md border-2 border-stone-100 flex flex-col">
                  ...
                  <ul className="space-y-3 overflow-y-auto flex-1 pr-2 no-scrollbar max-h-40">
                    {(stats?.weakWords || []).slice(0, showAllWeakWords ? undefined : 5).map((w: any, i: number) => (
                      <li key={i} className="flex justify-between items-center text-sm border-b border-stone-100 pb-2">
              ...
              {/* 30日間の学習推移（棒グラフ） */}
              <div className="bg-white p-6 rounded-[2rem] shadow-md border-2 border-stone-100 md:col-span-2">
...
<Bar dataKey="count" fill={adminTargetUser.id === 'sister' ? '#0ea5e9' : adminTargetUser.id === 'mami' ? '#ec4899' : adminTargetUser.id === 'kenta' ? '#10b981' : '#f59e0b'} radius={[4,4,0,0]} />
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="bg-rose-50/50 p-6 rounded-[2rem] border-2 border-rose-100 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-rose-100 outline-none focus:border-rose-300 font-bold bg-white" value={challengeSettings.reward_goal_days} onChange={e => setChallengeSettings({...challengeSettings, reward_goal_days: parseInt(e.target.value)})} />
                  </div>
                  <div>
              ...
              <div className="bg-emerald-50/50 p-6 rounded-[2rem] border-2 border-emerald-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-emerald-100 outline-none focus:border-emerald-300 font-bold bg-white" value={challengeSettings.quest_count} onChange={e => setChallengeSettings({...challengeSettings, quest_count: parseInt(e.target.value)})} />
                  </div>
                  <div>
                    <label className="text-xs font-black text-emerald-500 block mb-1 ml-2">週末ボス</label>
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-emerald-100 outline-none focus:border-emerald-300 font-bold bg-white" value={challengeSettings.special_quest_count} onChange={e => setChallengeSettings({...challengeSettings, special_quest_count: parseInt(e.target.value)})} />
                  </div>
                  <div>
                    <label className="text-xs font-black text-emerald-500 block mb-1 ml-2">パパ / Rick挑戦</label>
                    <input type="number" className="w-full p-4 rounded-2xl border-2 border-emerald-100 outline-none focus:border-emerald-300 font-bold bg-white" value={challengeSettings.challenge_quest_count} onChange={e => setChallengeSettings({...challengeSettings, challenge_quest_count: parseInt(e.target.value)})} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-amber-50/50 p-6 rounded-[2rem] border-2 border-amber-100 shadow-sm">
                ...
                <div className="bg-white p-4 rounded-2xl border-2 border-amber-100 mb-6">
                  <p className="text-[10px] font-black text-amber-400 mb-1 ml-1">自動選択の問題数（※自動モード時）</p>
                  <select className="w-full p-2 bg-transparent outline-none font-bold text-stone-700" value={challengeSettings.auto_count} onChange={e => setChallengeSettings({...challengeSettings, auto_count: parseInt(e.target.value)})}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => sendLineToChild(`【${adminTargetUser.name}】パパから新しい挑戦状が届いたよ！🔥\nクリアして「${challengeSettings.reward_text}」を目指そう！`)} className="bg-emerald-500 text-white font-black px-6 py-5 rounded-2xl shadow-xl active:scale-95 transition-all text-xs flex flex-col items-center justify-center gap-1"><span>LINE通知</span></button>
                </div>
              </div>
            </div>
          )}
          ...
          ========================================= */}
          {adminTab === 'add_word' && (
            <div className="bg-white p-8 rounded-[2.5rem] shadow-md border-2 border-stone-100 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="space-y-5">
                ...
                <textarea className="w-full border-2 border-stone-100 p-4 rounded-2xl font-bold outline-none focus:border-indigo-400 bg-stone-50 shadow-inner min-h-[120px]" placeholder="例文（意味や使い方がわかるように）" value={newWord.usage_example} onChange={e => setNewWord({...newWord, usage_example: e.target.value})} />
                <button onClick={onAddWordClick} className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg shadow-xl active:scale-95 transition hover:bg-indigo-700">データベースに追加する 🐾</button>
              </div>
            </div>
          )}
          ...
          ========================================= */}
          {adminTab === 'manage' && (
            <div className="bg-white p-6 rounded-[2rem] shadow-md border-2 border-stone-100 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  return (
                    <div key={w.id} className="bg-stone-50/50 rounded-2xl p-4 border-2 border-stone-50 shadow-sm flex items-center gap-4 group hover:border-sky-100 transition-all">
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         <button onClick={() => handleDeleteWord(w.id)} className="bg-white text-rose-400 p-2 rounded-xl border border-rose-50 hover:bg-rose-100 hover:text-rose-500 transition-all shadow-sm">
                           削除
                         </button>
                      </div>
                    </div>
